...
def team_monitor():
    ...
    common.page_title('Output Dashbord')
    ...
    st.markdown("<h2>📊 Output Today</h2>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='date-section'><h4>📅 DATE RANGE</h4></div>", unsafe_allow_html=True)
    _date = st.date_input(
        label=':calendar: :violet[**Select Date**]',
        key='b_date',
        ...
        try:
            ...
            for first_name, user_id in _users.items():
                ...
                with cols[i % 4]:
                    ...
                    df = df.rename(columns={'article_clean_url':'URL Listing'})
